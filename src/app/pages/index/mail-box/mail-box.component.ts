...
export class MailBoxComponent implements OnInit, OnDestroy {
  ...
  // Input properties for custom data fetching
  @Input() fetchMailDataFn?: (limit: number, offset: number) => Observable<any>;
  @Input() deleteMailFn?: (mailId: number) => Observable<any>;
  ...
  async loadMails() {
    if (!this.state.jwt() && !this.fetchMailDataFn) return;
    this.loading.set(true);
    try {
      let res: any;
      if (this.fetchMailDataFn) {
        res = await this.fetchMailDataFn(this.pageSize, (this.page() - 1) * this.pageSize).toPromise();
      } else {
        res = await this.api.getMails(this.pageSize, (this.page() - 1) * this.pageSize).toPromise();
      }
  ...
  async deleteSelected() {
    ...
    for (const mail of selected) {
      try {
        if (this.deleteMailFn) {
          await this.deleteMailFn(mail.id).toPromise();
        } else {
          await this.api.deleteMail(mail.id).toPromise();
        }
      } catch {}
    }
